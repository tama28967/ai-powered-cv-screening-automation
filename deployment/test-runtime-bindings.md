...
## 2026-08-20 — Error handling re-added (Founder direction), scoped to technical faults only, not business rejections

Founder requested error handling be added back to PRJ-0001, for legitimate reasons independent of the 2026-08-14 removal: that earlier removal was specifically about *not logging business-rejection outcomes* (duplicate/invalid-job/oversize/etc — these already have their own clean rejection branches and candidate-facing messages) to the `Applicant Records` sheet. Genuine **technical** faults (Drive/Sheets/Gemini API outages, unexpected exceptions) had no handling at all after that removal — a real gap, not the same thing the Founder asked to remove.

**Design — deliberately different shape from the old (deleted) Error Handling workflow**, which was called explicitly from 4 business-logic branches via `executeWorkflow`. This one uses n8n's **native `Error Trigger` node** + the pipeline's own `settings.errorWorkflow` binding — a catch-all for *any* unhandled node exception anywhere in W1, with zero explicit call sites to maintain (avoids the recurring "explicit call site forgot to carry a new field" maintenance trap already hit 3 times this project — INS-047 and prior).

- New workflow **`PRJ-0001 — Error Handling (Technical Faults)`** (`8oZ2rdMsRP6zWJbJ`), active: `Error Trigger` → `Build Error Log Entry` (Code node, extracts `workflow.name`, `execution.lastNodeExecuted`/`error.node.name`, `error.message`, `execution.url`, generates `error_id`/`timestamp`) → fans out to `Record Fault to Sheet` (Google Sheets append, `autoMapInputData`) and `Notify Admin` (Gmail, internal alert to `tama28967@gmail.com` — never candidate-facing).
- New sheet tab **`Error Log`** in the same spreadsheet (`1eiVPbSbJrZSiNa3FaKXgwQCJtxuew5zDeW7Qujgdd_8`), separate from `Applicant Records` by design — columns: `error_id, timestamp, workflow_name, failed_node, error_message, record_id, execution_url`. `record_id` is intentionally left blank for now: a generic Error Trigger payload doesn't carry application-level context, only workflow/node/error identity — acceptable for infra-fault visibility, a known limitation.
- W1 (`4nDrnkEQ9I9j8mjw`) `settings.errorWorkflow` set to `8oZ2rdMsRP6zWJbJ`.
- **Automatic retry added** (new — not present before) on the 8 most transient-failure-prone nodes: `Upload to Google Drive`, `AI Agent`, `Raw Gemini OCR Call`, `Write Record`, `Count Accepted for Job`, and all 3 Gmail notification nodes. `retryOnFail: true, maxTries: 3`, 2-3s backoff — directly targets the exact failure class already observed once in this project (a transient Gemini `503 Service Unavailable`, 2026-08-14, previously required a manual re-submit).

**Defect hit and fixed while building this:** the `Error Log` sheet's header row got corrupted twice during setup — first because `columns.mappingMode: "defineBelow"` on this n8n version now hard-requires an explicit `columns.schema` array (a stricter behavior than the `System Config` sheet fix earlier in this project), and second because a `Set`-node-fed `autoMapInputData` write against a sheet whose header was already polluted just matched the bad existing headers instead of creating correct ones. Root-caused by reading back actual execution node output (not the HTTP response, which was Cloudflare-cached and misleading) via `n8n_executions get`. Fixed by deleting and recreating the sheet tab empty, then doing the **first** write with `autoMapInputData` — n8n auto-derives the header row from the input item's field names on an empty sheet, which is the only reliable way observed so far. Reinforces the standing guidance (INS-036/INS-047 family): never trust a Sheets node's own returned JSON as proof of what was written — read the sheet back.

**Verified end-to-end, live:** a disposable webhook-triggered workflow was made to throw a synthetic error with W1's exact `errorWorkflow` wiring pattern. Real cascade confirmed via `n8n_executions get` on the Error Handling workflow's own execution: `Error Trigger` captured full context → `Build Error Log Entry` produced correct fields → `Record Fault to Sheet` appended a clean, correctly-headered row (`error_id: ERR-1787241260721`) → `Notify Admin` sent a real Gmail message (`id: 1a01fe177c547673`, `SENT`). All 3 disposable probe workflows used during this round were deleted afterward, per the established disposable-probe pattern.

**Not yet verified:** a real W1-internal failure (e.g. genuinely killing Drive/Sheets credentials mid-run) has not been forced — only the generic Error-Trigger-cascade mechanism was proven, via an external synthetic-throw workflow wired the same way. The mechanism itself is standard, well-documented n8n behavior (`errorWorkflow` + `Error Trigger`), so this is considered low-risk, but a real-fault live pass is recommended before the next Founder Manual Verification round, same caveat pattern used elsewhere in this file.

**Cleanup closed (same day, Founder-confirmed):** both disposable probe workflows (`TEMP — Create Error Log Sheet`, `TEMP — Error Trigger Test`) permanently deleted. The `PRJ-0001` n8n folder now holds exactly 2 live workflows — `PRJ-0001 — W1 Applicant Pipeline` and `PRJ-0001 — Error Handling (Technical Faults)` — plus the unrelated `PRJ-0002` workflow in the same instance, untouched.

Formalized as Evidence Record: **EV-048**.

## 2026-08-20/21 — error-handling verification round COMPLETED (resumed after MCP-disconnect interruption), all 4 steps PASS

Resumed via `/asdp-state PRJ-0001` after the prior session's `n8n-mcp` process hang was cleared by a session restart (`mcp__n8n__*` tools confirmed healthy first via `n8n_health_check`). Executed the exact 4-step technical resume plan the interrupted session had written (step 5, the Founder Acceptance decision itself, deliberately withheld — human-only gate, unchanged from the original ruling).

**Step 1 — full end-to-end dry run, PASS (execution 391).** A hand-built synthetic strong-match CV (`Dry Run Verify`, HTML/CSS/JavaScript/Bootstrap/React, 3y experience — clears `Front End Developer`'s bar of score≥90/2y) submitted via the real `Webhook Intake` route (local curl, multipart). All 29 executed nodes succeeded, zero retries fired on the healthy path, Gemini scored 100/shortlist, `Write Record` wrote the row, and a real Gmail acceptance send confirmed `SENT`. **Confirms the 2026-08-20 config-only changes (`errorWorkflow` binding + 8 `retryOnFail` node flags) introduced no regression** in the business flow proven across the 08-13/14/15 rounds.

**Step 2 — real retry forced, PASS (execution 392).** `Count Accepted for Job`'s `sheetName.value` temporarily mis-pointed (live `patchNodeField`, `retryOnFail`/`maxTries: 3` left untouched) to a nonexistent tab name. A second synthetic submission (`Retry Force Verify`) ran the node for **5,923ms** before failing — consistent with 3 attempts against `waitBetweenTries: 2000`, not a single fast failure — then the workflow correctly terminated `status: error` after retries exhausted (`NodeOperationError: Sheet with name ... not found`).

**Step 3 — genuine W1-internal cascade, PASS (execution 393).** Unlike the 2026-08-20 round's external synthetic-throw proof, this is the first real internal W1 failure to exercise the `errorWorkflow` binding. `PRJ-0001 — Error Handling (Technical Faults)` fired automatically: `Error Trigger` captured full context, `Build Error Log Entry` produced `workflow_name: "PRJ-0001 — W1 Applicant Pipeline..."` and `failed_node: "Count Accepted for Job"` exactly as required, `Record Fault to Sheet` appended a clean `Error Log` row (`ERR-1787245401622`), and `Notify Admin` sent a real Gmail alert (`id: 1a020209bd7771b3`, `SENT`).

**Step 4 — revert + final clean pass, PASS (execution 394).** `sheetName.value` reverted to `Applicant Records` (confirmed via `n8n_get_workflow` read-back before re-testing). One final fresh submission (`Final Clean Verify`) returned `{"status":"processed",...,"final_status":"accepted"}`, execution `394` `status: success` — W1 fully restored and healthy.

**TEST-data note:** 2 new accepted rows in `Applicant Records` (`Dry Run Verify`, `Final Clean Verify`), 1 new `Error Log` row (`ERR-1787245401622`), corresponding Drive files and Gmail sends — left in place, same disposition as every prior round's TEST artifacts.

**Verdict: mechanism verification PASS on all 4 technical steps.** Founder Manual Verification / Acceptance of this round — and of PRJ-0001's overall current shape — remains outstanding; not self-granted, per the original interrupted session's own ruling.

## 2026-08-21 — admin alert recipient moved to System Config sheet (Founder-directed, config-not-code)

Founder direction: the `Notify Admin` recipient must not be hardcoded in the workflow — it should live in a sheet a non-technical user can edit, same pattern already established for `active_language`.

**Change (`PRJ-0001 — Error Handling (Technical Faults)`, `8oZ2rdMsRP6zWJbJ`):** new row `admin_alert_email` added to the existing `System Config` sheet tab (same spreadsheet, same tab as `active_language`) — `config_value: tama28967@gmail.com`, `notes` explaining its purpose. Two new nodes inserted between `Build Error Log Entry` and `Notify Admin`: `Fetch Admin Alert Config` (Sheets read, single-condition filter on `config_key`, `alwaysOutputData: true` — identical pattern to W1's `Fetch Language Config`) → `Resolve Admin Email` (Code node, defaults to the original TEST address on any missing/blank config value — identical pattern to `Resolve Language`). `Notify Admin`'s `sendTo` now reads `={{ $json.admin_alert_email }}`; `subject`/`message` re-pointed to `$('Build Error Log Entry').first().json.*` since the node's immediate input is now the config-resolution branch, not the error data (same node-reference choke-point pattern used throughout this project for `lang`/`intake_channel`). `n8n_validate_workflow`: `errorCount: 0`, 6 nodes.

**Live-verified as genuinely dynamic, not coincidentally correct.** `admin_alert_email` was temporarily flipped (disposable probe) to a Gmail plus-address on the same inbox, `tama28967+admintest20260821@gmail.com` — a real W1-internal failure forced the same way as the earlier round (execution 397 → Error Handling execution 398): `Fetch Admin Alert Config` read the flipped value, `Resolve Admin Email` passed it through unchanged, and `Notify Admin` sent to that exact plus-address (confirmed `SENT`, `id: 1a0203665758f2de`) — proving the recipient is read live from the sheet, not the hardcoded fallback. Config reverted to `tama28967@gmail.com` immediately after, `Count Accepted for Job`'s deliberate mis-pointing reverted, one final clean submission confirmed W1 fully restored (`accepted`, execution consistent with prior clean runs).

**Founder impact:** the admin alert recipient can now be changed at any time by editing one cell in the `System Config` sheet — no workflow edit required. Two disposable probe workflows used for this round (`ZZ-DIAG-add-admin-alert-email-config`, `ZZ-DIAG-flip-admin-alert-config`) both deleted after use.
